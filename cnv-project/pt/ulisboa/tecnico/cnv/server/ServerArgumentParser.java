...
public class ServerArgumentParser {
    ...
    private void setupCLIOptions() {
        ...
        final Option addressOption = new Option(ServerParameters.ADDRESS.toString(),
                true, "server listen address (default: 127.0.0.1).");
        addressOption.setRequired(false);
        this.options.addOption(addressOption);

        final Option portOption = new Option(ServerParameters.PORT.toString(),
                true, "server listen port (default: 8000).");
        portOption.setRequired(false);
        this.options.addOption(portOption);

        final Option mapsDirectoryOption = new Option(ServerParameters.MAPS_DIR.toString(),
                true, "directory with maps on the server (default: WORKING_DIR/datasets).");
        mapsDirectoryOption.setRequired(false);
        this.options.addOption(mapsDirectoryOption);

        final Option outputDirOption = new Option(ServerParameters.OUTPUT_DIR_SHORT.toString(), ServerParameters.OUTPUT_DIR.toString(),
        ...
        final Option debugOption = new Option(ServerParameters.DEBUG_SHORT.toString(), ServerParameters.DEBUG.toString(),
